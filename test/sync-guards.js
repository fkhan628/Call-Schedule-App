...
const transientEmptyAutosave = {
  ...
  dataCounts: { schedule: 0, vacations: 0, noCallDays: 0, appShifts: 0 },
};
...
// ── dataCounts marker (mirror-retirement prep) ──
// The mirror keys (schedule/vacations/noCallDays) are write-only in the
// payload and headed for removal from buildPayload. These four checks pin
// the transition contract.
const strippedNoMarker = { ...transientEmptyAutosave };
delete strippedNoMarker.schedule;
delete strippedNoMarker.vacations;
delete strippedNoMarker.noCallDays;
delete strippedNoMarker.dataCounts;

// The failing-by-construction proof: real data may exist in schedule_weeks /
// time_off, but a payload stripped of the mirrors WITH NO MARKER still reads
// wiped — i.e. retiring the mirrors before the marker ships would classify
// every autosave as a wipe and block all saves. This is why the retirement
// PR must land AFTER this one. (Asserts current legacy behavior — green
// before and after the marker change.)
check("mirror-stripped payload, NO marker → wiped (retirement must wait for the marker)",
  payloadLooksWiped(strippedNoMarker) === true);

check("marker reports real data → NOT wiped even with mirrors stripped",
  payloadLooksWiped({ ...strippedNoMarker, dataCounts: { schedule: 14, vacations: 3, noCallDays: 1, appShifts: 20 } }) === false);

check("marker all-zero → still wiped (marker is not a blanket bypass)",
  payloadLooksWiped({ ...strippedNoMarker, dataCounts: { schedule: 0, vacations: 0, noCallDays: 0, appShifts: 0 } }) === true);

// Uncertainty must stay conservative at the save gate: a malformed marker
// adds nothing and the legacy verdict (wiped) stands.
check("malformed marker (NaN / non-numeric) → legacy verdict (wiped)",
  payloadLooksWiped({ ...strippedNoMarker, dataCounts: { schedule: NaN, vacations: "3", noCallDays: null } }) === true);
...
await (async () => {
  ...
  // Marker-only snapshot (a capture whose table folds BOTH failed): the
  // dataCounts marker says data existed, but nothing restorable is inside.
  // Restore must REFUSE on content — marker-accepting would hand the applier
  // an empty schedule with wipe authorization armed and delete every
  // schedule_weeks row.
  f = stubFetch([{ m: "GET", url: snapUrl, json: [{ id: 7, reason: "x", created_at: "c",
    data: { surgeons: INIT_SURGEONS, dataCounts: { schedule: 46, vacations: 11, noCallDays: 2, appShifts: 20 } } }] }]);
  applied = 0;
  r = await snapshots.restore(7, async () => { applied++; return { ok: true }; });
  check("marker-only snapshot (folds failed at capture) → REFUSED on content",
...
// dataCounts wipe-guard marker (mirror-retirement prep): buildPayload must
// stamp it (1 assignment site) and both blob-adoption arming blocks must
// consult it (2 sites). Section A tests the PREDICATE on fixtures; nothing
// executes buildPayload here, so these code-pattern pins are what keep the
// marker from silently vanishing from the payload. Comment-immune needles
// (full code patterns, not the bare identifier).
check("buildPayload stamps the dataCounts marker (1 assignment site)",
  count("p.dataCounts = {") === 1, `found ${count("p.dataCounts = {")}`);
check("both arming blocks consult the marker (2 sites)",
  count("d.dataCounts && [") === 2, `found ${count("d.dataCounts && [")}`);
